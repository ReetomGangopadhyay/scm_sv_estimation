...
time <- row.names(snp_data)[-1]  # remove first because of diff()

plot.ts(returns, main = "S&P 500 Returns (2018–2022)")




```{r}
# Use only the *synthetic post-event* returns
returns <- as.vector(synthetic_ts_post)

# Corresponding dates for labeling / plotting
time <- as.character(index(synthetic_ts_post))
