...
snp_returns <- snp_returns[-1]  # Remove NA from first row
dates <- index(snp_returns)  # Store time index
snp_returns <- as.vector(snp_returns)  # Convert to vector

# Define event date (modify this based on the actual event)
event_date <- as.Date("2020-03-15")  # Example: COVID-19 crash

# Split into pre-event and post-event periods
pre_event_returns <- snp_returns[dates < event_date]
post_event_returns <- snp_returns[dates >= event_date]

# Load additional market indices for synthetic control
...
# Convert to a common dataframe format
...
# Ensure time alignment with S&P 500 data
market_returns_df <- market_returns_df[index(market_returns_df) %in% dates, ]

# Define pre-event period for synthetic control training
pre_event_market_returns <- market_returns_df[dates < event_date, ]

# Create a properly formatted dataset for Synth package
synth_data <- data.frame(date = as.numeric(dates[dates < event_date]),
                         unit = rep(1, length(pre_event_returns)),  # Required for Synth
                         treated = as.vector(pre_event_returns),
                         market_returns_df[dates < event_date, ])

# Ensure time variables are numeric
synth_data$date <- as.numeric(synth_data$date)

# Data preparation for synthetic control
dataprep_obj <- dataprep(
  foo = synth_data,
  predictors = colnames(market_returns_df),  # Use market indices as predictors
  dependent = "treated",
  unit.variable = "unit",  # Required: Each observation belongs to a single unit
  time.variable = "date",
  treatment.identifier = 1,  # Treated unit is S&P 500
  controls.identifier = colnames(market_returns_df),
  unit.names.variable = "unit",  # This must exist
  time.predictors.prior = unique(synth_data$date),
  time.optimize.ssr = unique(synth_data$date)
)
...
synthetic_ts <- xts(synthetic_returns, order.by = dates[dates < event_date])

# Compare actual vs. synthetic returns
plot(index(pre_event_returns), pre_event_returns, type = "l", col = "red", lwd = 2, 
     main = "Actual vs. Synthetic S&P 500 Returns", ylab = "Returns", xlab = "Time")
lines(index(synthetic_ts), synthetic_ts, col = "blue", lwd = 2)
legend("topright", legend = c("Actual S&P 500", "Synthetic Control"), 
       col = c("red", "blue"), lwd = 2)

# Store synthetic control values
synthetic_returns_full <- c(synthetic_returns, post_event_returns)

# Convert to dataframe for further analysis
synthetic_df <- data.frame(Date = dates, 
                           Actual = snp_returns, 
                           Synthetic = synthetic_returns_full)
...
# Convert market returns to dataframe and align dates
...
snp_df <- data.frame(Date = as.numeric(dates), SP500 = snp_returns)
...
# Assign numeric identifiers to units
...
synthetic_ts <- xts(synthetic_returns, order.by = as.Date(dates[dates < event_date]))

# Plot actual vs synthetic returns
plot(index(snp_returns[dates < event_date]), snp_returns[dates < event_date], type = "l", col = "red", lwd = 2, 
     main = "Actual vs. Synthetic S&P 500 Returns", ylab = "Returns", xlab = "Time")
lines(index(synthetic_ts), synthetic_ts, col = "blue", lwd = 2)
legend("topright", legend = c("Actual S&P 500", "Synthetic Control"), 
       col = c("red", "blue"), lwd = 2)
...
synthetic_df <- data.frame(Date = dates[dates < event_date], 
                           Actual = snp_returns[dates < event_date], 
...
# Set date range: Jan 1, 2018 to Jun 15, 2020
start_date <- as.Date("2018-01-01")
end_date <- as.Date("2022-03-15")
event_date <- as.Date("2020-03-16")  # COVID-19 crash date

# Get S&P 500 data and restrict date range
snp_data <- getSymbols('^GSPC', src='yahoo', auto.assign=FALSE)
...
# Get and restrict market data for control group
...
# Merge all market returns into one dataframe
market_returns_df <- do.call(merge, market_returns)
colnames(market_returns_df) <- indices

# Align dates and remove rows with missing values
market_returns_df <- market_returns_df[index(market_returns_df) %in% dates, ]
market_returns_df <- na.omit(market_returns_df)

# Update dates after NA removal
dates_filtered <- index(market_returns_df)
snp_filtered <- snp_returns[dates %in% dates_filtered]
...
# Merge S&P 500 with control indices
full_data <- merge(snp_df, market_returns_df, by = "Date", all.x = TRUE)
...
# Filter to complete units (balanced panel)
obs_count <- long_data %>%
  group_by(Unit) %>%
  summarise(n_obs = n())
...
# Reassign numeric IDs
long_data$Unit_ID <- as.numeric(factor(long_data$Unit))

# Pre-event data for Synth
pre_event_data <- long_data[long_data$Date < as.numeric(event_date), ]
pre_event_data <- pre_event_data[order(pre_event_data$Date, pre_event_data$Unit_ID), ]
pre_event_data$Unit <- as.character(pre_event_data$Unit)
...
# Plot actual vs synthetic S&P 500
plot(dates_filtered[dates_filtered < event_date], snp_filtered[dates_filtered < event_date],
     type = "l", col = "red", lwd = 2,
     main = "Actual vs. Synthetic S&P 500 Returns (Pre-COVID)",
     ylab = "Returns", xlab = "Date")
lines(index(synthetic_ts), synthetic_ts, col = "blue", lwd = 2)
abline(v = as.Date(event_date), col = "gray", lty = 2)
legend("topright", legend = c("Actual S&P 500", "Synthetic Control"),
       col = c("red", "blue"), lwd = 2)
...
# Set date range: Jan 1, 2018 to Mar 15, 2022
start_date <- as.Date("2018-01-01")
end_date <- as.Date("2022-03-15")
event_date <- as.Date("2020-03-16")  # First weekday after COVID crash trigger
...
# Load additional indices for synthetic control
indices <- c("^DJI", "^IXIC", "^RUT")  # Removed FTSE and N225 due to calendar gaps
...
# Create time series of actual and synthetic returns
actual_ts <- xts(snp_filtered, order.by = dates_filtered)
synthetic_pre <- dataprep_obj$Y0 %*% synth_res$solution.w
synthetic_ts <- xts(synthetic_pre, order.by = dates_filtered[dates_filtered < event_date])

# Extract pre- and post-event actual returns
...
plot(index(actual_pre), actual_pre, type = "l", col = "red", lwd = 2,
     ylim = range(c(actual_ts, synthetic_ts)),
...
legend("topright", legend = c("Actual S&P 500 (Pre)", "Synthetic Control", "Actual S&P 500 (Post)"),
       col = c("red", "blue", "red"), lwd = 2, lty = c(1, 1, 2))
