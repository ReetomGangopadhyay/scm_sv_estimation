...
data <- data[paste0(event_date, "/", end_date)]

# Get log returns
returns <- diff(log(Cl(data)))
returns <- returns[-1]  # Remove NA created by diff
returns <- as.vector(returns)

# Store S&P 500 data for plotting
snp_data <- as.data.frame(data)
time <- row.names(snp_data)[-1]  # drop the first row to match returns length

rm(data)

plot.ts(returns, main = "S&P 500 Returns (Post-Event)", ylab = "Log Returns", col = "firebrick", lwd = 2)
```
